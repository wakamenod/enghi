...
calendar_shortcut      = "enghi-events"   # macOS: カレンダーを読むショートカットの名前
calendar_sync_interval = "30m"            # 同期が on のとき、何分ごとに実行するか
```
...
カレンダーの予定を表示できる（読み取りのみ）。ダッシュボードには今日の予定が、作業記録には
その日の予定が出る。インターネットアカウントで macOS のカレンダー App に追加した
Google カレンダーも対象になる。カレンダーを読む許可はショートカット App が持つため、
予定はショートカットで読み取り、enghi がそれを 30 分ごとに実行する。使い始めるには、
**設定 → カレンダー** で **ショートカットを追加** を押し、ショートカット App で一度実行して
アクセスを許可してから、同期を on にする。予定の **タスクにする** を押すと、時刻入りの名前の
タスクができ、予定の日の Scheduled に入る。ショートカットを手で作る手順と、うまくいかない
ときの対処はガイド (`/guide/enghi#calendar`) にある。ほかのツールからは、OS を問わず
`PUT /api/calendar/events` で予定を送れる。

## キーボードショートカット (Web UI)

`/` 検索 · `g d`/`g w`/`g i`/`g n`/`g p` 移動 · `c` クイックキャプチャ ·
`e` 編集 · `j`/`k` リスト移動 · `Enter` 開く · `o` タスクの URL を開く ·
`Esc` 閉じる
