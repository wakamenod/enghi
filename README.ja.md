...
カレンダーの予定を、読み取り専用で表示できる。ダッシュボードに今日の予定が、作業記録に
その日の予定が出る。インターネットアカウントで macOS のカレンダー App に追加した
Google カレンダーも含む。カレンダーへのアクセス許可はショートカット App が持つので、
予定はショートカットで読み取り、enghi が 30 分ごとに実行する。**設定 → カレンダー** で
**ショートカットを追加** を押し、ショートカット App で一度実行してアクセスを許可してから、
同期を on にする。予定の **タスクにする** を押すと、時刻を名前に入れたタスクが、予定の日に
Scheduled でできる。ショートカットを手で作る手順と、うまくいかないときの対処は
ガイド (`/guide/enghi#calendar`) にある。ほかのツールからは、どの OS でも
