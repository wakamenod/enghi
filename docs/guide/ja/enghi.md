...
入力に URL（`http://` か `https://`）が含まれていると、タイトルから取り除いてタスクの **URL** にします。「これを読む https://example.com/post」と入れると、そのページにつながった「これを読む」というタスクになります。
...
- **URL** には、Issue や記事などタスクに関係する Web ページを入れます。URL のあるタスクは一覧に 🔗 が付きます。🔗 をクリックするか、タスクを選んで `o` を押すと新しいタブで開きます。`http://` と `https://` だけを受け付けます。
