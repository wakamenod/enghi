...
入力に URL（`http://` か `https://` で始まるもの）があると、タイトルから外してタスクの URL にします。`これを読む https://example.com/post` と入れると、「これを読む」というタスクができて、URL にそのページが入ります。
...
- **URL** には、Issue や記事など、タスクに関係する Web ページを入れます。使えるのは `http://` と `https://` だけです。URL があるタスクは、一覧に 🔗 が付きます。🔗 をクリックするか、タスクを選んで `o` を押すと、新しいタブで開きます。
...
作業中のタスクを Inbox、Later、Waiting、Scheduled、Someday へ移すと、自動で中断されます。作業ログには「⏸ 中断（Someday へ移動）」のように残るので、中断を押し忘れても、いつまでも作業中のままにはなりません。移動を元に戻すとこの中断も取り消され、作業中に戻ります。Next へ移したときは作業中のままです。
...
**作業記録** は、1日にやったことをまとめて見る画面です。`g` `l` か、ダッシュボードの「今日の作業 →」で開きます。終えたタスクと手を付けたタスクが並び、各タスクの下には、その日の作業ログが付きます。各行はタスクの行なので、`j` / `k` で移動でき、タスクのキーや移動ダイアログもほかの一覧と同じように使えます。操作のあとは同じ日に戻ります。

開いたときは今日の記録が出ます。ほかの日を見るには、右の月カレンダーで日付を選ぶか、`[` と `]` で前後の日へ移ります。`t` で今日に戻り、日付欄に入力して移ることもできます。カレンダーでは、記録のある日に点が付きます。

1日の記録は次の4つに分かれます。

- **完了** — その日に完了したタスクと、完了した時刻。あとで完了を取り消したタスクは出ない。何日かかけて終えたタスクには、その日より前の記録も直近3件まで「それ以前の記録」として日付付きで付き、何を終えたのかが分かる。長いときは畳んである。
...
ほかに、ダッシュボードにはいま作業中のタスクが並びます。それぞれに「10:42 から (2時間5分)」のように開始時刻と経過時間が付き、前日以前に始めたものには日付も付きます。中断を押し忘れていても、ここで気づけます。また、Weekly Review からはレビュー当日より前の7日間の各日の作業記録を開けます。「先週完了したもの」と同じ7日間です。

## カレンダーの予定（macOS） {#calendar}

カレンダーの予定を表示できます。enghi は予定を読むだけで、書き換えません。ダッシュボードの一番上には今日の予定が、作業記録の一番上にはその日の予定が並びます。macOS のカレンダー App にあるカレンダーなら何でも使え、システム設定 → インターネットアカウントで追加した Google カレンダーも対象です。

予定は `enghi-events` という名前のショートカットが読み取ります。カレンダーを読む許可はショートカット App が持っているので、enghi を更新しても許可を求められることはありません。enghi は 30 分ごとにこのショートカットを実行し、昨日から 14 日後までの予定を保存します。それより前の日の予定も消さずに残すので、過去の作業記録にもその日の予定が出ます。

使い始める手順:

1. Google カレンダーを使う場合は、システム設定 → インターネットアカウントでアカウントを追加し、カレンダーを on にします。カレンダー App に予定が出ることを確かめてください。
2. 設定 → カレンダーで **ショートカットを追加** を押します。ショートカット App が開いて `enghi-events` を追加するか尋ねてくるので、**ショートカットを追加** を押します。
3. ショートカット App でこのショートカットを開き、一度実行します。カレンダーへのアクセスを求められたら許可します。
4. 設定に戻って **カレンダーを同期する** を on にし、保存します。最初の同期はすぐに始まります。

終日の予定が先に並び、そのあとに時刻のある予定が時刻順に続きます。予定にはそれぞれカレンダー名が付きます。今日の予定では、始まった予定の下に「現在」の線が入り、終わった予定は薄くなります。どちらも再読み込みしなくても時計に合わせて動きます。画面に出したくないカレンダーは、**表示するカレンダー** でチェックを外します。同期は続くので、チェックを入れ直せばすぐに予定が戻ります。

予定の **タスクにする** を押すと、その予定がタスクになります。名前には予定の時刻が入り、「15:00–16:00 企画会議」のようになります。タスクは予定の日の Scheduled に入り、その日になると Next Actions に出ます。終日の予定から作ったタスクには時刻が入りません。タスクを作ったあとは、予定からそのタスクへのリンクが付きます。

ほかのツールからも、OS を問わず予定を送れます。`PUT /api/calendar/events?source=名前&from=YYYY-MM-DD&to=YYYY-MM-DD` に、ショートカットの出力と同じ形の JSON 配列を送ります。

```json
[{"title": "企画会議", "start": "2026-09-28T15:00:00+09:00", "end": "2026-09-28T16:00:00+09:00", "calendar": "職場", "location": "会議室A"}]
```

送った配列で、その source の予定のうち指定した日に始まるものを置き換えます。終日の予定なら、`start` と `end` は `2026-09-28` のような日付だけでもかまいません。このとき `end` は最終日を表します。`GET /api/calendar/events?from=&to=` を使うと、保存した予定を取り出せます。表示しないカレンダーの予定は含まれません。

**うまくいかないとき**

- 「ショートカットが追加されていません」と出る: 設定からもう一度追加します。別の名前で作った場合は、設定ファイルの `calendar_shortcut` をその名前にします。
- ショートカットのメッセージが出て同期に失敗する: たいていはカレンダーへのアクセスをまだ許可していません。ショートカット App を開いて一度実行し、アクセスを許可してから **今すぐ同期** を押します。
- エラーは出ないのに予定が出ない: ショートカットは何も見つけていません。結果が空でもエラーにはなりません。カレンダー App に予定が出ているか、**表示するカレンダー** でそのカレンダーにチェックが入っているかを確かめてください。
- ショートカットを追加できない: 次の節の手順に従って自分で作ります。

## カレンダーのショートカットを手で作る {#calendar-manual}

**ショートカットを追加** がうまくいかないときは、ショートカット App で同じものを作れます。項目名は macOS のバージョンによって少し違うことがあります。

1. 新しいショートカットを作り、名前を `enghi-events` にします（一字一句このとおりに）。
2. **カレンダーの予定を検索** を追加します。**フィルタを追加** を押し、**開始日** ·「**が次の過去の期間内**」· **2** · **日** にします。並び順は **開始日**、**古い順** のままにします。
3. **変数に追加** を追加し、入力を「カレンダーの予定」、変数名を `events` にします。
4. 2つ目の **カレンダーの予定を検索** を追加し、フィルタを **開始日** ·「**が次の未来の期間内**」· **14** · **日** にします。続けて、もう一度 `events` への **変数に追加** を置きます。2つの条件は別々のアクションにしてください。「いずれか」で1つのアクションにまとめると、何も見つからなくなります。
5. **各項目を繰り返す** を追加し、対象を変数 `events` にします。
6. 繰り返しの中に **辞書** を追加し、種類がテキストの項目を5つ作ります。
   - `title`: 変数 **繰り返し項目**
   - `start`: **繰り返し項目**。押して **開始日** を選び、**日付のフォーマット** を **ISO 8601** にして、**時刻を含める** を on にする
   - `end`: `start` と同じ手順で **終了日** を選ぶ
   - `calendar`: **繰り返し項目** の **カレンダー**
   - `location`: **繰り返し項目** の **位置**
7. 繰り返しの中で続けて **テキスト** を追加し、変数 **辞書** だけを入れます。辞書をテキストにすると、1行の JSON になります。
8. **繰り返しの終了** の後に **テキストを結合** を追加し、対象を **繰り返しの結果**、区切りを **改行** にします。
9. **停止して出力** を追加し、出力を **結合済みのテキスト** にします。
10. 一度実行して、カレンダーへのアクセスを許可します。ターミナルで `shortcuts run enghi-events < /dev/null` を実行し、予定1件ごとに1行の JSON が出れば完成です。`shortcuts run` は標準入力が端末でないとき、その入力が終わるまで待ちます。スクリプトやパイプから実行して止まったように見えるときは、`< /dev/null` を付けてください。
