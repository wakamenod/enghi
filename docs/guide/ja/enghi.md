...
カレンダーの予定を、読み取り専用で表示できます。ダッシュボードの一番上に今日の予定が、作業記録の一番上にその日の予定が並びます。対象は macOS のカレンダー App にあるカレンダーです。**システム設定 → インターネットアカウントで追加した Google カレンダーも含みます。**

予定は、`enghi-events` という名前のショートカットを通して取り込みます。カレンダーを読む許可はショートカット App が持つので、enghi を更新しても許可を求め直されることはありません。enghi は 30 分ごとにこのショートカットを実行し、昨日から 14 日後までの予定を保存します。それより前の予定は残るので、過去の日の作業記録にも、その日の予定が出ます。

設定の手順:

1. Google カレンダーを使うなら、システム設定 → インターネットアカウントでアカウントを追加し、カレンダーを on にします。カレンダー App に予定が出ることを確かめてください。
2. 設定 → カレンダーで **ショートカットを追加** を押します。ショートカット App が開き、`enghi-events` を追加するか尋ねるので、**ショートカットを追加** を押します。
3. ショートカット App でこのショートカットを開き、一度実行します。カレンダーへのアクセスを求められたら許可します。
4. 設定に戻り、**カレンダーを同期する** を on にして保存します。最初の同期はすぐに走ります。

予定は、終日の予定が先、続いて時刻のある予定が時刻順に並び、それぞれにカレンダー名が付きます。今日の予定では、始まった予定の後ろに「現在」の線が入り、終わった予定は薄く表示されます。どちらも再読み込みなしで時計に合わせて動きます。**表示するカレンダー** でチェックを外すと、そのカレンダーは画面に出なくなります。同期は続けているので、チェックし直せば予定はすぐ戻ります。

予定の **タスクにする** を押すと、「15:00–16:00 企画会議」のように時刻を名前に入れたタスクができ、予定の日に Scheduled になります。その日になると Next Actions に出ます。終日の予定から作ったタスクには時刻が付きません。作ったあとは、予定に「タスク」のリンクが付きます。

ほかのツールからも予定を送れます。macOS 以外でも使えます。`PUT /api/calendar/events?source=名前&from=YYYY-MM-DD&to=YYYY-MM-DD` に、ショートカットが出力するのと同じ形の JSON 配列を送ります。

```json
[{"title": "企画会議", "start": "2026-09-28T15:00:00+09:00", "end": "2026-09-28T16:00:00+09:00", "calendar": "職場", "location": "会議室A"}]
```

その source の予定のうち、指定した日に始まるものを置き換えます。終日の予定なら `start` と `end` は日付だけ（`2026-09-28`）でもよく、`end` は最終日です。`GET /api/calendar/events?from=&to=` で、保存した予定を取れます。表示しないカレンダーは除かれます。

**うまくいかないとき**

- 「ショートカットが追加されていません」: 設定からもう一度追加します。別の名前で作ったなら、設定ファイルの `calendar_shortcut` をその名前にします。
- ショートカットのメッセージ付きで同期に失敗する: たいていはカレンダーへのアクセスを許可していません。ショートカット App で一度実行してアクセスを許可し、**今すぐ同期** を押します。
- 予定が出ないのにエラーもない: ショートカットが何も見つけていません。空の結果はエラーではありません。カレンダー App に予定が出ているか、そのカレンダーが「表示するカレンダー」でチェックされているかを確かめてください。
- ショートカットを追加できない: 次の節の手順で、自分で作ります。

## カレンダーのショートカットを手で作る {#calendar-manual}

**ショートカットを追加** がうまくいかないときは、ショートカット App で同じものを作れます。項目の名前は macOS のバージョンによって少し違うことがあります。

1. 新しいショートカットを作り、名前をちょうど `enghi-events` にします。
2. **カレンダーの予定を検索** を追加します。**フィルタを追加** を押し、**開始日** ·「**が次の過去の期間内**」· **2** · **日** にします。並び順は **開始日**、**古い順** のままにします。
3. **変数に追加** を追加し、入力を「カレンダーの予定」、変数名を `events` にします。
4. 2つ目の **カレンダーの予定を検索** を追加し、フィルタを **開始日** ·「**が次の未来の期間内**」· **14** · **日** にします。続けて、もう一度 `events` への **変数に追加** を置きます。**2つの条件を「いずれか」で1つのアクションにまとめないでください。** 何も見つからなくなります。
5. **各項目を繰り返す** を追加し、対象を変数 `events` にします。
6. 繰り返しの中に **辞書** を追加し、種類がテキストの項目を5つ作ります。
   - `title`: 変数 **繰り返し項目**
   - `start`: **繰り返し項目**。押して **開始日** を選び、**日付のフォーマット** を **ISO 8601**、**時刻を含める** を on にします。
   - `end`: 同じように **終了日** で。
   - `calendar`: **繰り返し項目** の **カレンダー**。
   - `location`: **繰り返し項目** の **位置**。
7. 繰り返しの中に、続けて **テキスト** を追加し、変数 **辞書** だけを入れます。テキストにすると、辞書は1行の JSON になります。
8. **繰り返しの終了** の後に **テキストを結合** を追加し、対象を **繰り返しの結果**、区切りを **改行** にします。
9. **停止して出力** を追加し、**結合済みのテキスト** を出力にします。
10. 一度実行し、カレンダーへのアクセスを許可します。ターミナルで `shortcuts run enghi-events` を実行すると、予定1件につき1行の JSON が出るはずです。
