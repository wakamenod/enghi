...
**作業記録** は、1日にやったことをまとめて見る画面です。`g` `l` か、ダッシュボードの「今日の作業 →」で開きます。終えたタスクと手を付けたタスクが並び、各タスクの下には、その日の作業ログが付きます。

開いたときは今日の記録が出ます。ほかの日を見るには、右の月カレンダーで日付を選ぶか、`[` と `]` で前後の日へ移ります。`t` で今日に戻り、日付欄に入力して移ることもできます。カレンダーでは、記録のある日に点が付きます。

1日の記録は次の4つに分かれます。

- **完了** — その日に完了したタスクと、完了した時刻。あとで完了を取り消したタスクは出ない。
- **作業中** — その日の終わりの時点で、開始したまま中断していなかったタスク。今日なら、いま作業中のもの。過去の日は、開始と中断の記録から割り出す。
- **手を付けた** — 上の2つ以外で、その日に作業ログを書いたタスク。
- **取りやめ** — その日に破棄したタスク。定期タスクで飛ばした回も含む。最初は畳んである。

「Markdown でコピー」を押すと、その日の記録を Markdown でコピーできます。日報やチャットへの貼り付けに使えます。Emacs の中など、ブラウザがコピーに対応していない場所では、テキストが選択された状態で表示されるので、手でコピーします。

同じ内容は `/api/day?date=YYYY-MM-DD` からも取れます。JSON で返り、`&format=markdown` を付けると Markdown になります。Claude Code のスキルに日報を頼むと、これを読んで下書きを作ります。

ほかに、ダッシュボードにはいま作業中のタスクが並び、Weekly Review からは先週の各日の作業記録を開けます。
...
| `e` | 記事を編集 |
| `[` / `]` | 作業記録で前の日 / 次の日へ |
| `t` | 作業記録で今日へ戻る。タスクを選択しているときは、その題名を変える |
